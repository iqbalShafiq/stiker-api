...
async function main() {
  ...
  if (!userRole || !adminRole) {
    throw new Error('Required roles are missing after seed upsert');
  }

  const allPermissions = await prisma.permission.findMany();

  const userPermissionNames = ['sticker:read', 'sticker:write', 'sticker:delete', 'sticker:share', 'user:read', 'user:write'];
  const userPermissions = allPermissions.filter((permission) => userPermissionNames.includes(permission.name));
